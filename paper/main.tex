...
nor a claim that a minimal thermal-qubit repeated-interaction engine exists.
\end{abstract}
...
fresh probes~\cite{Bettmann2023,Prositto2025,CollisionalTransmon2025}.

Recent work proves that a \emph{minimal qubit} under Markovian repeated
interactions with thermal hot and cold ancillas cannot act as a refrigerator
or as a heat engine: the cycle-total work is nonpositive, and the setup
reduces to heat conduction~\cite{NoGo2026}. That no-go is important, but it
applies to a different question from ours. We do not run a qubit between two
thermal reservoirs and ask for engine or fridge operation. We use a V-type
system whose coherence is renewed, and we export that resource into the
ergotropy of a battery tape via matched-gap exchange. Their result therefore
does not forbid our construction; it instead motivates going beyond the
minimal thermal-qubit repeated-interaction machine---exactly the setting we
study.
...
The scope is narrow on purpose. We do not claim a two-bath heat engine that
evades Ref.~\cite{NoGo2026}. We do not claim that exporting coherence
restores a collinear spin-valve current. We do not claim that coherent
ergotropy is free work without a phase reference. We do not treat ergotropy
as an extra energy current in the first law.
...
It is useful to contrast our figure of merit with that of
Ref.~\cite{NoGo2026}. There the working fluid is a single qubit, the ancillas
are thermal, and the thermodynamic question is whether the repeated
interaction with hot and cold baths can yield refrigeration or positive
cycle-total work. The answer is negative. Here the working medium is a
three-level system with a renewed coherent resource, the ancillas are
typically prepared near the ground state as batteries, and the collision is
chosen to be energy conserving on the active block. The quantity we
optimize is the ergotropy rate of the outgoing tape, not the cycle work of a
minimal qubit heat engine. In that sense we study one of the enriched
repeated-interaction settings that Ref.~\cite{NoGo2026} points to, rather
than a counterexample to their no-go. If the enrichment is switched off---a
single qubit, only thermal collisions, and the same work accounting---one
must recover constraints of that type; we do not claim otherwise.
...
energy $\Delta F_A$ must enter any net device efficiency. A phase reference
used at discharge is likewise a resource. Gross $P_{\rm erg}$ is therefore
an upper bound on continuous collisional charging from renewed coherence, not
a complete two-bath engine efficiency. These points do not change the
collision map or the impedance match. They do limit the thermodynamic claim.
...
positivity checks, and a real electrical-power/ergotropy tradeoff. Known
minimal-qubit no-go results constrain the limits of that program when
particle flow, bias, and the battery tape are removed~\cite{NoGo2026}. The
full spin-valve analysis is not part of the present companion result.
...
two-copy activation follows the thresholds in Eq.~(\ref{eq:twocopy}). The
construction sits outside the minimal thermal-qubit repeated-interaction
setting of Ref.~\cite{NoGo2026}; it does not claim to overturn that no-go.
These distinctions put the coherence-harvesting bound on a clear
